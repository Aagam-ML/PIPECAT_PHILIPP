...
class VoiceChatClient {
  constructor() {
    ...
    // IMPORTANT: this must match a key in your Python VOICE_PROFILES,
    // e.g. "ADVERTISING", "FRIENDLY", "SERIOUS", etc.
    this.selectedProfile = 'ADVERTISING';
    this.voiceButtons = null;
  ...
  setupDOM() {
    ...
    // Grab the voice option buttons from the DOM
    this.voiceButtons = document.querySelectorAll('.voice-option');
  ...
  // Setup the voice selector UI and wiring
  setupVoiceSelector() {
    if (!this.voiceButtons || this.voiceButtons.length === 0) {
      console.warn('No .voice-option buttons found in DOM');
      return;
    }

    // Set initial active state in UI
    this.setActiveVoiceButton(this.selectedProfile);

    this.voiceButtons.forEach((btn) => {
      btn.addEventListener('click', () => {
        const profile = btn.dataset.profile;
        if (!profile) return;

        this.selectedProfile = profile;
        this.setActiveVoiceButton(profile);
        this.addEvent('voice-selected', profile);

        // Only send if client is connected AND bot is ready
        if (this.client && this.isConnected && this.botReady) {
          try {
            this.client.sendClientMessage('set-voice', { profile });
            this.addEvent('voice-sent', `Sent set-voice: ${profile}`);
          } catch (err) {
            console.error('Failed to send set-voice:', err);
            this.addEvent('error', `Failed to send set-voice: ${err.message}`);
          }
        }
      });
    });
  ...
  setActiveVoiceButton(profile) {
    if (!this.voiceButtons) return;
    this.voiceButtons.forEach((btn) => {
  ...
  onBotReady() {
    ...
    // Send current voice selection to the bot once it is truly ready
    if (this.selectedProfile) {
      try {
        this.client.sendClientMessage('set-voice', {
          profile: this.selectedProfile,
        });
        this.addEvent(
          'voice-sync',
          `Sent initial voice: ${this.selectedProfile}`
        );
        console.log('Initial voice sent after bot-ready');
      } catch (err) {
        console.error('Failed to send initial set-voice at bot-ready:', err);
        this.addEvent('error', `Failed to sync voice: ${err.message}`);
      }
    }
  }
